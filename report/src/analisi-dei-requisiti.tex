...
\subsubsection[R-explore]{\requirementref{R-explore}}\labelsssec{R-explore}
...
\subsubsection[R-startExplore]{\requirementref{R-startExplore}}\labelsssec{R-startExplore}

L'esplorazione deve partire in seguito all'invio di un comando da parte dell'operatore.

\subsubsection[R-TempOk]{\requirementref{R-TempOk}}\labelsssec{R-TempOk}

L'esplorazione deve partire solo se la temperatura rispetta le condizioni prestabilite.

\subsubsection[R-stopExplore]{\requirementref{R-stopExplore}}\labelsssec{R-stopExplore}
...
\subsubsection[R-backHome]{\requirementref{R-backHome}}\labelsssec{R-backHome}

Il robot deve tornare alla posizione iniziale se richiesto dall'utente al seguito dello stop dell'esplorazione.

\subsubsection[R-continueExplore]{\requirementref{R-continueExplore}}\labelsssec{R-continueExplore}

Il robot deve continuare l'esplorazione se richiesto dall'utente.

\subsubsection[R-blinkLed]{\requirementref{R-blinkLed}}\labelsssec{R-blinkLed}

Il LED del robot deve lampeggiare se esso è in fase di esplorazione.

\subsubsection[R-consoleUpdate]{\requirementref{R-consoleUpdate}}\labelsssec{R-consoleUpdate}

La console deve contenere le informazioni aggiornate riguardo allo stato di esplorazione del robot.

\subsubsection[R-stopAtBag]{\requirementref{R-stopAtBag}}\labelsssec{R-stopAtBag}

Il robot deve fermarsi in prossimità di una valigia.

\subsubsection[R-takePhoto]{\requirementref{R-takePhoto}}\labelsssec{R-takePhoto}

Il robot deve scattare una fotografia all'ostacolo appena individuato.

\subsubsection[R-sendPhoto]{\requirementref{R-sendPhoto}}\labelsssec{R-sendPhoto}

Il robot deve inviare la fotografia appena scattata alla console.

\subsubsection[R-alert]{\requirementref{R-alert}}\labelsssec{R-alert}

Il device dell'operatore, una volta ricevuta la foto, deve essere in grado di riconoscere se la valigia contiene una bomba, ed, in questo caso, inviare una notica all'operatore.

\subsubsection[R-storePhoto]{\requirementref{R-storePhoto}}\labelsssec{R-storePhoto}

La fotografia deve essere salvata in modo permanente contestualmente alle informazioni relative.

\subsubsection[R-backHomeSinceBomb]{\requirementref{R-backHomeSinceBomb}}\labelsssec{R-backHomeSinceBomb}

In caso la valigia contenga un ordigno, il robot Discovery deve tornare immediatamente alla posizione iniziale.

\subsubsection[R-continueExploreAfterPhoto]{\requirementref{R-continueExploreAfterPhoto}}\labelsssec{R-continueExploreAfterPhoto}
...
\subsubsection[R-waitForHome]{\requirementref{R-waitForHome}}\labelsssec{R-waitForHome}

Il sistema è in attesa che il robot discovery torni alla base.

\subsubsection[R-reachBag]{\requirementref{R-reachBag}}\labelsssec{R-reachBag}
...
\subsubsection[R-bagAtHome]{\requirementref{R-bagAtHome}}\labelsssec{R-bagAtHome}
